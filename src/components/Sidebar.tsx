    ...
    <>
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-violet-500/10">
          ...
          <button className="lg:hidden p-1.5 rounded-lg hover:bg-violet-500/10" onClick={onClose}>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          <p className="px-4 py-2 text-[10px] font-semibold uppercase tracking-widest text-violet-400/40">Navigation</p>
          {navLinks.map((link) => {
            ...
            return (
              <motion.button
                ...
                className={`sidebar-link w-full text-left ${isActive ? 'active' : 'text-slate-400'}`}
              >
                <link.icon className={`w-[18px] h-[18px] ${isActive ? 'text-violet-400' : ''}`} />
                {link.label}
                {link.label === 'SOS' && (
        ...
        <div className="px-3 py-4 border-t border-violet-500/10 space-y-1">
          ...
          <button className="sidebar-link w-full text-left text-slate-500">
        ...
        <div className="px-4 py-4 border-t border-violet-500/10">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-semibold" style={{ background: 'linear-gradient(135deg, #8B5CF6, #EC4899)' }}>
              A
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-200 truncate">Anonymous User</p>
              <p className="text-xs text-slate-500 truncate">Protected Identity</p>
            </div>
          </div>
        </div>
      </aside>
