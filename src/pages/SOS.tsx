...
const defaultContacts: Contact[] = [
  { id: '1', name: 'Emergency Services', phone: '100', relationship: 'Police', is_primary: true },
  { id: '2', name: 'Women Helpline', phone: '1091', relationship: 'Helpline', is_primary: true },
  { id: '3', name: 'Mom', phone: '+91-98765-43210', relationship: 'Family', is_primary: false },
  { id: '4', name: 'Priya (Friend)', phone: '+91-91234-56789', relationship: 'Friend', is_primary: false },
];
...
export default function SOS() {
  ...
  useEffect(() => {
    async function fetchContacts() {
      try {
        const { data } = await supabase.from('trusted_contacts').select('*').limit(10);
        if (data?.length) setContacts([...defaultContacts, ...data.map(d => ({ ...d, is_primary: false }))]);
      } catch {
        // use defaults
      }
    }
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="text-center">
        ...
        <p className="text-sm text-slate-500 mt-1">One tap to alert your trusted contacts and emergency services</p>
      </div>
          ...
          {!sosSent ? (
            ...
            >
              ...
              {sosActive && (
                <>
                  ...
                  <div className="absolute w-56 h-56 rounded-full border border-rose-500/20 pulse-ring" style={{ animationDelay: '0.5s' }} />
                  <div className="absolute w-64 h-64 rounded-full border border-rose-500/10 pulse-ring" style={{ animationDelay: '1s' }} />
                </>
              )}
              ...
              {sosActive && countdown !== null && (
                <motion.button
                  ...
                  style={{ background: 'rgba(15,23,42,0.6)', border: '1px solid rgba(139,92,246,0.2)' }}
                >
                  Cancel Alert
                </motion.button>
              )}
              ...
              {!sosActive && (
                <p className="mt-6 text-sm text-slate-500">Press and hold to send emergency alert</p>
              )}
            </motion.div>
          ) : (
            <motion.div
            ...
            >
              <motion.div
                ...
                style={{ background: 'linear-gradient(135deg, rgba(52,211,153,0.2), rgba(16,185,129,0.1))', border: '2px solid rgba(52,211,153,0.3)' }}
              >
                <CheckCircle className="w-12 h-12 text-emerald-400" />
              </motion.div>
              <h2 className="text-xl font-bold text-emerald-400 mb-2">SOS Alert Sent</h2>
              <p className="text-sm text-slate-400 text-center max-w-sm">
                Emergency alerts have been sent to {contacts.length} contacts. Help is on the way.
              </p>
              <div className="flex items-center gap-2 mt-4 text-xs text-slate-500">
                <MapPin className="w-3 h-3 text-violet-400" />
                Location shared: {location ? `${location.lat.toFixed(4)}, ${location.lng.toFixed(4)}` : 'Acquiring...'}
              </div>
              <button
                onClick={handleReset}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { icon: Phone, label: 'Call 100', color: 'from-rose-500 to-red-600', action: () => window.open('tel:100') },
          { icon: Phone, label: 'Women Helpline', color: 'from-pink-500 to-rose-600', action: () => window.open('tel:1091') },
          { icon: Volume2, label: 'Alarm Sound', color: 'from-amber-500 to-orange-600', action: () => {} },
          { icon: Send, label: 'Share Location', color: 'from-violet-500 to-purple-600', action: () => {} },
        ].map((action) => (
          <motion.button
            key={action.label}
          ...
          >
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center bg-gradient-to-br ${action.color} opacity-80`}>
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-slate-200">Trusted Contacts</h2>
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={handleAddContact} className="btn-primary text-white px-4 text-xs">Add</button>
                </div>
              </div>
            </motion.div>
          )}
      ...
      >
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-base font-semibold text-slate-200">Safety Tips</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {[
            ...
            <div key={i} className="flex items-start gap-2 p-3 rounded-xl" style={{ background: 'rgba(139,92,246,0.03)' }}>
